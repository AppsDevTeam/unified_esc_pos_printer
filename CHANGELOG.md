...
- **USB scan no longer offers devices that cannot print.** A POS terminal's USB bus carries root hubs, internal hubs, network adapters and card readers alongside the printer; all of them used to show up in the picker, and picking one is not harmless — a serial driver probing a hub resets it and takes every device behind it, the printer included, off the bus. The new `UsbScanFilter` (passed as `UsbConnector(scanFilter: ...)`) defaults to `printerCandidatesOnly`, which drops hubs and devices with no bulk OUT endpoint. `UsbScanFilter.all` restores the previous unfiltered listing for the case where a printer is not recognised and the user has to pick it by hand. Devices the platform did not describe are always kept, so the filter can never hide a printer it failed to classify.
- **USB Printer class devices now connect straight to raw bulk transfer.** A device exposing interface class 7 speaks plain bulk transfer with no serial protocol underneath, so the six serial driver candidates could never open it — they only ran, failed, and left the raw fallback to succeed last. Each of those probes claimed the interface and pushed control transfers at a device that does not understand them. The connector now classifies the device up front and takes the transport that can actually work.
- **The serial type probe is bounded and stops on a permission error.** Each candidate now gets 3 s; previously a driver stuck on a control transfer had nothing to cut it short. A `PERMISSION_DENIED` result aborts the probe instead of being swallowed and retried with the next candidate, which raised a fresh system permission dialog every time.
- **Connecting to a USB hub is refused outright**, even when the caller reached one past the scan filter.
- **ASB status is logged on change, not on every packet.** Printers that push ASB periodically rather than only on change repeat the same "everything is fine" packet every few seconds forever, which buried every other line in the log and read like a busy loop. The first packet and every actual change are still logged, and the status stream is unchanged — it still receives every packet.
- Requires `usb_serial` v0.9.0, which exposes the USB descriptor data the classification is based on and moves the blocking USB calls off the main thread.

## 3.8.4

- **BLE write no longer hangs forever on a mid-print disconnect (iOS + Android).** When the printer dropped the link while a write was in flight — powered off mid-job, moved out of range — the native write's result was silently dropped, so the Dart-side `await writeBytes` never completed and the caller's UI (e.g. a print spinner) hung indefinitely with no error ever surfacing. Both platforms now fail the in-flight write with a `DISCONNECTED` error the instant the disconnect is detected, propagating as `PrinterWriteException`.
  - **iOS** (`BleManager.swift`): `write()` had no timeout and `cleanupConnection` removed the pending `FlutterResult` maps without invoking them. Pending writes are now completed with an error on disconnect, plus an 8 s per-write backstop timeout for the rarer "silent stall with no disconnect event" case. A one-shot `settled` guard prevents double-resolution across the CoreBluetooth callback, the timeout, and the disconnect cleanup.
  - **Android** (`BleManager.kt`): `onConnectionStateChange(STATE_DISCONNECTED)` completed a pending `connectResults` (connect in flight) but ignored `writeResults` (write in flight). It now also fails any in-flight write; the `remove()`-based handoff with `onCharacteristicWrite` — both on the main looper — guarantees the result is delivered exactly once.
